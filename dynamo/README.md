...
## Supported Hardware
The package is designed for Intel RealSense DXXX cameras. 
...
[calibration.py](calibration.py) - Calibrates connected RealSense devices to a single global coordinate system, using a template chessboard
...
**example gif of dynamic body shape capture**
...
Duck brand **link** auto reflective tape was tested with this package. 
Markers must be a minimum of 1cm x 1cm to be reliabily detected by the included algorithms. 

**picture of reflective tape**
...
See [notebooks](notebooks/notebooks.md) for a list of Jupyter notebooks which explain how the each function works in this package. 

## Testing

The package includes a script which tests the installation of the `Dyanamo` package. 
After downloading the `Dynamo` package, run `python testscript.py` inside the `Dynamo` package to view a sample scene using the `Dynamo` package.
...
This package is licensed under the Apache License, Version 2.0. Copyright 2019 XXXXX
